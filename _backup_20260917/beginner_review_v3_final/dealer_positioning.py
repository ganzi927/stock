...
def implied_forward_from_chain(
    chain: pd.DataFrame, spot: float, r: float, t: float, atm_band: float = 0.075
) -> float | None:
    """풋-콜 패리티로 옵션 체인에서 내재 선도가격 F를 역산한다 (OptionMetrics IvyDB /
    CBOE 방식). C(K) − P(K) = e^(−rT)·(F − K) 이므로 등가격(±atm_band) 행사가들의
    (C−P) 를 K 에 선형회귀하면 기울기 ≈ −e^(−rT), F = −절편/기울기.

    거래소 선물 종가는 근월물·비유동 구간에서 스팟과 스냅샷이 어긋나 내재배당을 크게
    왜곡시킨다 — 같은 스냅샷의 유동적인 ATM 옵션에서 뽑는 편이 안정적이다. 회귀가
    불안정하면(기울기 부호/크기 이상) 최근접 ATM 한 행사가 공식으로 폴백한다.
    """
    if spot <= 0 or t <= 0:
        return None
    calls = chain[chain["type"] == "C"][["strike", "close"]].rename(columns={"close": "c"})
    puts = chain[chain["type"] == "P"][["strike", "close"]].rename(columns={"close": "p"})
    both = pd.merge(calls, puts, on="strike", how="inner").dropna()
    both = both[(both["c"] > 0) & (both["p"] > 0)]
    if both.empty:
        return None

    def _single_atm(m: pd.DataFrame) -> float:
        row = m.loc[(m["strike"] - spot).abs().idxmin()]
        return float(row["strike"] + np.exp(r * t) * (row["c"] - row["p"]))

    band = both[(both["strike"] / spot - 1.0).abs() <= atm_band]
    if len(band) < 4:
        return _single_atm(both)

    slope, intercept = np.polyfit(band["strike"].to_numpy(float), (band["c"] - band["p"]).to_numpy(float), 1)
    disc = np.exp(-r * t)
    if slope < -1e-9 and abs(slope / (-disc) - 1.0) <= 0.10:
        return float(-intercept / slope)
    return _single_atm(band)
...
def build_scenarios(levels: Levels) -> list[dict]:
    """호환 함수명. 가격 경로 대신 정의와 위치가 있는 관측 레벨을 반환한다."""
    wall_limit = "감마·IV 등 모형 입력에 의존하며 실제 딜러 순포지션이나 지지·저항을 식별하지 못합니다."
    pain_limit = "내재가치 합의 통계량이며 만기 가격 수렴이나 가격 유지 조건을 뜻하지 않습니다."
    candidates = [
        ("Call Wall", levels.call_wall, "탐색 범위의 콜 감마 가중 OI 최대 행사가", wall_limit),
        ("상단 Gamma Wall", levels.call_wall_above, "종가 위 콜 감마 가중 OI 최대 행사가", wall_limit),
        ("Put Wall", levels.put_wall, "탐색 범위의 풋 감마 가중 OI 최대 행사가", wall_limit),
        ("하단 Gamma Wall", levels.put_wall_below, "종가 아래 풋 감마 가중 OI 최대 행사가", wall_limit),
        ("Zero Gamma", levels.zero_gamma, "가정된 순감마 프로파일의 영점", "계약별 부호·IV 가정에 의존하며 실제 딜러 포지션은 미확인입니다."),
        ("MaxPain (부분 체인·비표준)", levels.max_pain, "선택된 부분 체인의 만기 내재가치 합 최소 행사가", pain_limit),
        ("MaxPain (전 행사가·계약)", levels.max_pain_full, "전체 체인의 만기 내재가치 합 최소 행사가", pain_limit),
    ]
    observations = []
    for name, value, definition, limitation in candidates:
        if value is None or not np.isfinite(value):
            continue
        distance = (value / levels.spot - 1) * 100
        observations.append(dict(name=name, value=float(value), distance_pct=distance,
            position="위" if distance > 0 else "아래" if distance < 0 else "동일",
            definition=definition, limitation=limitation))
    return observations
