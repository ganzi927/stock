...
def _beginner_model_explanation(
    spot: float,
    levels: Levels,
    pot: dict[str, float],
    risk_reversal: tuple[float | None, float | None, float | None] | None,
    ...
    def describe(label: str, value: float | None, key: str | None = None) -> None:
        ...
        where = "위" if pct > 0 else "아래" if pct < 0 else "같은 위치"
        distance = f"{abs(pct):.1f}% {where}" if pct else where
        pot_text = ""
    ...
    describe("Zero Gamma(합산 감마 부호가 바뀌는 계산 경계)", levels.zero_gamma, "zero_gamma")
    describe("MaxPain(부분 체인의 내재가치 합이 가장 작은 행사가)", levels.max_pain, "max_pain")
    if risk_reversal is not None and risk_reversal[2] is not None:
        rr = risk_reversal[2]
        premium = "풋 IV가 콜 IV보다 높습니다" if rr < 0 else "콜 IV가 풋 IV보다 높습니다" if rr > 0 else "콜·풋 IV가 같습니다"
        sentences.append(f"25델타 Risk Reversal은 {rr:+.2f}%p로 {premium}. 이는 옵션 가격의 비대칭이지 현물 방향 예측이 아닙니다.")
    if positioning_skew and positioning_skew.get("oi_skew") is not None:
        oi_skew = positioning_skew["oi_skew"] * 100
