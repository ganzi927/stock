...
class QualityTests(unittest.TestCase):
    ...
    def test_options_default_no_ai_research_leak(self):
        ...
        self.assertEqual(facts['usage'],'context');self.assertIn('maturity_band',facts)
    ...
    def test_outlook_uses_ai_with_allowlist_and_falls_back(self):
        ...
        with patch.object(outlook,'generate_commentary',return_value='AI가 관측 맥락을 설명했습니다.') as ai:
            page=outlook.build_outlook_section(ASOF,fgi,[facts,dict(facts,title='STALE',as_of='2026-09-14')])
        ai.assert_called_once()
        prompt=ai.call_args.args[0]
        self.assertIn('AI가 관측 맥락을 설명했습니다.',page)
        self.assertIn('Claude AI 종합 해설',page)
        for forbidden in ('SECRET','99999','STALE','상승 추진력','방어수요'):
            self.assertNotIn(forbidden,prompt)

        with patch.object(outlook,'generate_commentary',return_value=None):
            fallback=outlook.build_outlook_section(ASOF,fgi,[facts])
        self.assertIn('자동 요약',fallback)
        self.assertIn('250,000.0 원',fallback)
        self.assertIn('풋/콜 OI 비율: 0.50',fallback)

    def test_outlook_shows_curated_maturity_boundaries_for_every_product(self):
        levels=options_report.Levels(
            spot=1000.,max_pain=777.,dex_neutral_maxpain=None,zero_gamma=950.,
            call_wall=1200.,put_wall=800.,strike_min=700.,strike_max=1300.,
            call_wall_above=1100.,put_wall_below=900.,
        )
        research=outlook.section_facts(
            'fixture','기준일 2026-09-15 · 만기 2026-10-08 (T-23)',1000.,levels,
            build_scenarios(levels),0.,0.,0.,None,None,None,
        )
        self.assertEqual(research['maturity_band']['upper']['name'],'상단 Gamma Wall')
        self.assertEqual(research['maturity_band']['lower']['name'],'Zero Gamma')
        self.assertNotEqual(research['maturity_band']['lower']['value'],777.)
        fgi=dict(total=60.,trend=40.,sentiment=70.,included=['Momentum'],
                 indicators={'Momentum':40.},quality={'Momentum':{'usage':'context'}})
        titles=['코스피200 옵션 (정규월물)','코스피200 위클리 옵션','삼성전자 옵션','SK하이닉스 옵션']
        facts=[]
        for title in titles:
            fact=dict(usage='context',title=title,as_of=ASOF.isoformat(),spot=1000.,spot_unit='원',
                      observations=dict(call_oi=100,put_oi=50,call_volume=20,put_volume=30))
            fact.update(research);fact['title']=title;facts.append(fact)
        with patch.object(outlook,'generate_commentary',return_value='네 상품의 관측 맥락입니다.'):
            page=outlook.build_outlook_section(ASOF,fgi,facts)
        for title in titles:
            self.assertIn(title,page)
        for expected in ('만기 2026-10-08 (T-23)','상단 관찰 레벨','1,100.0 원',
                         '하단 관찰 레벨','950.0 원','연구용 계산','목표가·손절가'):
            self.assertIn(expected,page)
        self.assertNotIn('777.0 원',page)
