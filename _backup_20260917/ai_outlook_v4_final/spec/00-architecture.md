...
- `outlook`(종합 상태)은 결정론적 관측 설명이며 API 키/LLM 스위치와 무관하다.
  `fgi_facts`가 None이거나 기준일 일치 옵션 관측이 없으면 생략한다.
...
`reports/<anchor>.meta.json`의 `options_complete == true`이고 품질 정책 및
`REPORT_CONTRACT_VERSION`이 모두 현행이면 즉시 `exit 0`. 구버전/누락 버전은 재생성한다.
...
report_contract_version : int               (표시·해석 계약 버전, 현행 4)
...
- **AC-ARCH-5.** 품질 정책만 일치하는 구버전 meta는 재사용하지 않는다. 두 버전과 완결 상태가 모두 맞을 때만 재사용한다.
