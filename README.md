> **품질 게이트 v2:** [QUALITY_GUIDE.md](QUALITY_GUIDE.md)를 먼저 읽어주세요. 기본 합성 구성 변경, 연구용 분리, 공식 기준자료 입력과 검증 명령을 설명합니다.

> **2026-09-16 방법론 감사:** 확인된 계산·단위·해석 오류를 수정했습니다. 현재 한계와 판단 필요 항목은 [METHODOLOGY_AUDIT.md](METHODOLOGY_AUDIT.md)를 먼저 읽어주세요. 기존 조사 기록이나 타 리포트와의 수치 일치는 예측력 검증이 아닙니다.
...
운영용 통합 리포트는 `python combined_main.py` (아래 "매일 자동 실행" 참고).

### 매일 자동 실행 / 기준일 고정

`combined_main.py`가 운영 진입점이다. 리포트는 **하나의 거래일(anchor)에 완전히
고정**된다 — anchor = `--as-of` 이하 마지막 거래일(기본 오늘). 공포탐욕지수·옵션·
개별주식·헤더·종합전망이 전부 그 날 마감 기준.

```bash
python combined_main.py                     # 최신 거래일
python combined_main.py --as-of 2026-09-07  # 그 거래일 기준으로 (재)생성
python combined_main.py --force             # 완결 리포트가 있어도 다시 생성
```

- **엄격 게이트**: anchor 당일 옵션 EOD가 KRX Open API에 아직 없으면(야간 배치라
  T일 20:00엔 대개 없음) 리포트를 만들지 않고 `exit 3`. 이전 거래일 데이터로 조용히
  폴백하지 않는다. `reports/<anchor>.pending.json`에 시도 횟수만 기록.
- **멱등**: 같은 anchor의 완결 리포트가 이미 있으면 AI 재호출 없이 즉시 종료.
- 생성 성공 시 `reports/<anchor>.meta.json` 사이드카(anchor, fgi_as_of, option_bas_dd,
  options_complete, generated_at)를 남긴다.

자동화는 `register_task.ps1`로 Windows 작업 스케줄러에 등록한다: 평일 20:00~23:00
30분 간격(대개 옵션 데이터가 없어 "대기") + **매일 08:00**(KRX 야간 배치 후 그날 첫
실행 — 여기서 전날 거래일 리포트가 완성됨) + 부팅 3분 후. `run_daily.ps1`은
`combined_main.py`를 호출하기만 한다(판단은 전부 그 안에서).

08:00 실행 시점에도 KRX 옵션 EOD가 아직 안 올라왔으면 그날 리포트는 다음 트리거(저녁
20:00)까지 대기한다. 리포트가 없는 걸 확인했을 때 기다리지 않으려면 저장소 루트의
**`리포트생성.cmd`를 더블클릭**하면 `combined_main.py`를 즉시 1회 돌리고, 성공하면
방금 만든 리포트를 브라우저로 연다(옵션 EOD가 아직이면 "잠시 후 다시" 안내만 뜸).
...
상수(r=3.0%, q=1.5%)로 대체한다 — 각 섹션 제목 옆에 어느 쪽을 썼는지 "(시장)"/"(패리티내재)"/
"(선물내재)"/"(가정)"으로 표시된다.
- **r**: KRX Open API의 `3개월무위험금리 선물`(`fut_bydd_trd`) SPOT_PRC(=100−금리)에서 계산
- **q**: 업계 표준(OptionMetrics IvyDB / CBOE 방식)에 맞춰 `options_main._resolve_rq`가
  다음 순서로 정한다:
  1. **풋-콜 패리티 내재 선도가**: 등가격(±7.5%) 행사가들의 `C−K − P` 를 K에 회귀해
     `F* = −절편/기울기` (`dealer_positioning.implied_forward_from_chain`). 같은 스냅샷의
     유동적인 ATM 옵션을 쓰므로 거래소 선물 종가보다 스냅샷 오염이 적다. → `q = r − ln(F*/S)/T`
  2. 실패 시 옵션 만기에 근접한 **선물 종가**로 역산 (`F=S·e^((r−q)T)`)
  3. **잔존만기 < 10일 & 만기 안에 지수 배당락일 없음**이면, 짧은 T 탓에 연율화 q가
     스냅샷 노이즈로 폭주하므로(예: 2026-09-07 만기주에 선물기준 −47%, 패리티기준 +29%)
     신뢰하지 않고 상수로 폴백 → "(가정)"
  4. 그 외에도 q가 타당 범위를 벗어나면 상수로 폴백. 타당 범위는 만기 내 배당락 유무로
     달라진다 — 한국 상장사는 배당이 연말에 몰려서, 만기가 12월 배당락
     (`dividends.INDEX_EXDIV_DATES`)을 걸치면 연율 15~40% q도 정상이라 넓게(±50%),
     아니면 KOSPI200 상시 수준(−2%~+6%)만 허용
...
spec/           - **모듈별 정식 사양(normative spec).** 각 계산의 입력·수식·불변조건·
                  수용 기준(테스트 연결)·미해결 이슈. 계산 로직을 바꾸면 같은 커밋에서
                  해당 spec 파일도 갱신한다. 인덱스: spec/README.md
```
