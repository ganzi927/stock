...
#   - 매일 08:00 보정 (핵심): KRX 야간 배치가 올라온 뒤 그날 첫 실행. 여기서 전날 거래일
#       리포트가 완성된다. (06:00 → 07:30 → KRX 게시 지연 누적으로 08:00 재상향.
#       2026-09-08·09 기준 KRX 옵션 EOD 게시가 08:00~09:00대였음.)
#       08:00에도 아직 없으면 사용자가 리포트생성.cmd 를 더블클릭해 수동 보완한다.
#   - 부팅 3분 후 (08:00에 PC가 꺼져 있다가 그 뒤에 켠 경우, 켜자마자 한 번 시도)
...
#         최대 절전(hibernate)/완전 종료는 못 깨움 → 08:00 / 부팅 트리거가 커버.
...
$tMorning = New-ScheduledTaskTrigger -Daily -At 8:00AM
