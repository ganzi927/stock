...
def build_section_html(
    ...
    level_rows += _level_row("Zero Gamma", levels.zero_gamma, spot, pot=pot.get("zero_gamma"))
    level_rows += _level_row("일반 MaxPain", levels.max_pain, spot, pot=pot.get("max_pain"))
    ...
    <div class="card-grid" style="margin-top:14px">
      <div class="card"><div class="card-name">Net DEX</div><div class="card-score" style="color:{'#3D5C36' if net_dex>=0 else '#8A4A47'}">{_eok(net_dex)}</div><div class="card-note">딜러 순델타 명목가치(원, delta×OI×승수×스팟) — 고객 콜·풋 순매수 → 딜러 콜·풋 순매도 가정</div></div>
      <div class="card"><div class="card-name">Vanna Flow</div><div class="card-score" style="color:{'#3D5C36' if net_vex>=0 else '#8A4A47'}">{_eok(net_vex)}</div><div class="card-note">IV 1%p 상승 시 델타 명목가치 변화(원)</div></div>
      <div class="card"><div class="card-name">Charm Flow</div><div class="card-score" style="color:{'#3D5C36' if net_charm>=0 else '#8A4A47'}">{_eok(net_charm)}</div><div class="card-note">하루 경과 시 델타 명목가치 변화(원) — 시간가치 소멸</div></div>
    </div>"""
    ...
    <div class="section-block">
      ...
      <table class="level-table">
        <tr><th>레벨</th><th>가격</th><th>스팟 대비</th><th>PoT(만기 전 터치확률)</th></tr>
        {level_rows}
      </table>
      {commentary_html}
...
<div class="footer">
  ...
  선물에서 실시간 역산한 값, "(가정)"은 데이터가 없을 때 쓰는 상수(r=3.0%, q=1.5%). 딜러는 콜 순매수/풋 순매도로
  가정하는 표준 소매 관례를 따름 — 실제 딜러 포지션과 다를 수 있음.<br/>
