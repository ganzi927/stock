...
BOND_ETF_CODE = "148070"  # KOSEF 국고채10년 — CNN Safe Haven Demand는 장기 국채 기준
                          # (fmkorea도 국채 10년물). 예전 114260(KODEX 국고채3년)에서 교체.
...
def generate_fgi_section() -> tuple[str, float, float, float | None, dict | None]:
    ...
    # 정규화 창(PCT_WINDOW=504 ≈ 2년)에 맞춰 시세 캐시도 ~2.5년 확보한다.
    HIST_DAYS = 620
    ...
    bond_etf = update_cache(CACHE_DIR / "bond10y.csv", fetch_item_history, BOND_ETF_CODE, min_days=HIST_DAYS)
    ...
    safe_haven_df = ind.compute_safe_haven(kospi200, bond_etf)
