...
## §0.1 프로젝트 정체 (2026-08-31 확정 — 표류 금지)

**이 프로젝트는 "코스피 대형주(코스피200) + 반도체 등 주도주의 딜러 포지셔닝·심리를
서술하는 맥락 readout"이다.** 광의 시장 공포탐욕지수가 아니고, CNN/fmkorea 복제도 아니며,
방향 예측 도구도 아니다. 코스닥·소형주·거시지표는 범위 밖(매크로 배경으로만 참고).

이 정체에서 따라 나오는 확정 결정 (웹·백테스트 재검증 완료):

| 항목 | 결정 | 근거 |
|---|---|---|
| Strength·Breadth 유니버스 | **일자별 시총 상위 200** (`STRENGTH_UNIVERSE_N=200`) | 광의 등락 breadth는 소형주 편향 → 대형주 목표엔 틀린 대상을 잼 (StockCharts/Fidelity) |
| KOSDAQ | **편입 안 함** | 대형주 범위 밖. `ksq_bydd_trd` 이용신청도 보류 |
| Credit Spread | **TOTAL·투심에서 제외, 카드로만 표시** (`indicators.MACRO_BACKDROP`) | 한국 BBB- 시장 2026 동결 → 매트릭스 프라이싱 계단함수(연 11스텝), 거시 변수, IC는 위기 2회 의존 |
| Volatility 평활 | **VKOSPI 20일 MA** (`VKOSPI_MA_DAYS=20`) | CNN 50일은 일간 심리엔 둔함. 반응성 근거의 의도적 이탈 — **백테스트 IC로 고른 값 아님** (CI가 겹쳐 서열화 불가) |
| 정규화 창 | **252일 유지** | CNN 표준, "최근 1년 국면 대비". IC 스윕으로 고르지 않음 |
| 개별주식 옵션 GEX | **삼성전자·SK하이닉스만 풀 모델** | 단일종목 GEX는 최유동 종목에서만 유효(SpotGamma). 주도주 추가는 최소 OI 기준 통과 시에만, 아니면 경량 표시 |
| 주도주 유니버스 | 손으로 안 고름 — **규칙·시점(point-in-time) 적용** | 생존편향·룩어헤드. 모멘텀은 생존편향과 특히 나쁘게 상호작용 |

IC 백테스트는 **가드레일**(지표 방향이 뒤집혔나 분기 점검)이지 **옵티마이저**(스펙 선택자)가
아니다. 이 데이터셋(2016~2026 단일 국면, n_eff 20~40, 중첩수익률)은 스펙을 결정적으로
서열화할 수 없다.
