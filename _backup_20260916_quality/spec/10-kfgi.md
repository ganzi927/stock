...
- raw = 코스피200 옵션(정규) `put_vol / call_vol`의 `PUTCALL_MA_DAYS = 5`일 이동평균.
  `invert=True`(풋 매수 우위 = 공포).
...
- **투심지수 sentiment** = `mean(Volatility, Put/Call Ratio)`. 역행 지표 — 점수↑ =
  변동성·풋수요 낮음 = 안심/방심, 점수↓ = 헤지수요 큼 = 공포.
