...
레벨을 스팟 위/아래로 정리한 **나열**이지 방향 예측이 아니다. 3종:
- **Bullish Squeeze**: `upper_target` = `call_wall`(스팟 위) 또는 `call_wall_above`.
  invalidation = `zero_gamma`(<spot) → `put_wall` → `put_wall_below`.
- **Base Case (박스권)**: `max_pain` 부근 유지. invalidation = 상/하단 wall 이탈.
- **Bearish Regime Shift**: trigger = `zero_gamma`, target = `put_wall`/`put_wall_below`.
- 각 `note`의 딜러 헤지 방향은 "부호 가정이 맞다면"의 조건부. `invalidation`은 손절가가
  아니라 그 구조가 깨졌을 때의 다음 반대편 구조적 레벨.
